...
def countAllele(_ref, _seq):
	alleleDic = {"A":0, "T":0, "G":0, "C":0, "In":0, "Del":0, "N":0, 'InSeq':'', 'DelSeq':''}
	JumpCnt = 0
	InSeq = {}
	DelSeq = {}
	...
	for Seq in InSeq.keys() :
		if not Seq == '' :
			if alleleDic["InSeq"] :
				...
				alleleDic["InSeq"] = "%s:%d" % (Seq, InSeq[Seq])
	for Seq in DelSeq.keys() :
		if not Seq == '' :
			if alleleDic["DelSeq"] :
				alleleDic["DelSeq"] += ",%s:%d" % (Seq, DelSeq[Seq])
			else :
				alleleDic["DelSeq"] = "%s:%d" % (Seq, DelSeq[Seq])
			
	return str(alleleDic["A"]), str(alleleDic["T"]), str(alleleDic["G"]), str(alleleDic["C"]), str(alleleDic["N"]), str(alleleDic["In"]), str(alleleDic["Del"]), alleleDic["InSeq"], alleleDic["DelSeq"]
...
if __name__ == "__main__" :
	...
	for line in fi_pileup.xreadlines():
		...
		if len(words) < 5 :
			...
			seq = words[4]
			quality = words[5]
			a,t,g,c,n,inser,delet,inseq,delseq = countAllele(words[2].upper(), seq)
			...
			bestAllele = ''
			if int(a) > int(t) and int(a) > int(g) and int(a) > int(c) and int(a) > int(n) :
				bestAllele = 'A'
			elif int(t) > int(a) and int(t) > int(g) and int(t) > int(c) and int(t) > int(n) :
				bestAllele = 'T'
			elif int(g) > int(a) and int(g) > int(t) and int(g) > int(c) and int(g) > int(n) :
				bestAllele = 'G'
			elif int(c) > int(a) and int(c) > int(t) and int(c) > int(g) and int(c) > int(n) :
				bestAllele = 'C'
			else :
				bestAllele = 'N'
			if not delseq == '' :
