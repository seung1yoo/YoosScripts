...
def make_go_dic_v3(goseq_results):
    go_dic = dict()
    for fn in goseq_results:
        for line in open(fn):
            items = line.rstrip('\n').split('\t')
...
def make_goseq_dic(goseq_results):
    ...
    for fn_result in goseq_results:
        ...
        print deg_name
        if len(deg_name.split('/')[-1].split('.')) in [1]:
            deg_pair = deg_name.split('/')[-1]
            updown = 'updown'
        else:
...
def uploader(name_dic, fn_report, goseq_results):
    ...
    for goseq_result in goseq_results:
        if not os.path.exists('./Upload/{0}'.format(goseq_result)):
            print(goseq_result)
            os.symlink(os.path.abspath(goseq_result), './Upload/{0}'.format(goseq_result.split('/')[-1]))
    #
...
def main(args):
    if args.go_enrich:
        ...
        go_dic = make_go_dic_v3(args.goseq_results)
