...
def expFilter(xls, log2fc, statistics, value, title_key, probe_key, gene_key, desc_keys):
    ...
    for line in open(xls):
        ...
        if items[0] in [title_key]:
            ...
            for desc_key in desc_keys:
                desc_idxs.append(items.index(desc_key))
            exp_idxs = [i for i, item in enumerate(items) if re.search(':RPKM$', item)]
            ...
            if statistics in ['p']:
                v_idxs = [i for i, item in enumerate(items) if re.search(':P$', item.strip("\""))]
                v_cols = [items[x] for x in v_idxs]
            elif statistics in ['q']:
                v_idxs = [i for i, item in enumerate(items) if re.search(':Q$', item.strip("\""))]
                v_cols = [items[x] for x in v_idxs]
            continue
        #
...
if __name__=='__main__':
    ...
    parser.add_argument('--anno-table', default='Databases/Ontology_Archives/gene_ontology_edit.obo.2019-12-01.table')
    parser.add_argument('--xls', default='genes.deg.xls')
    parser.add_argument('--outdir', default='gsea')
    parser.add_argument('--prefix', default='outprefix')
    ...
    parser.add_argument('--sample-info', default='RootControl6h:RootControl,RootControl12h:RootControl,RootControl24h:RootControl,RootPA016h:RootPA01,RootPA0112h:RootPA01,RootPA0124h:RootPA01')
    parser.add_argument('--primarykeys', nargs='+', help='Title_key GS_key Probe_key Gene_key Desc_keys(Name@Desc)',
            default=['GeneId', 'ANNO:GO', 'GeneId', 'GeneId', 'Symbol@Description'])
